...
namespace customMD{
    public class HTML{
        ...
        private Mark header_mark;
        private Mark footer_mark;
        private Mark main_mark;
        ...
        public Mark initialize(){
            ...
            this.header_mark = this.body_mark.addSChildMark(null, MarkType.div);
            this.main_mark = this.body_mark.addSChildMark(null, MarkType.div);
            this.footer_mark = this.body_mark.addSChildMark(null, MarkType.div);
            this.header_mark.addProperty(PropertyType._class, "header");
            this.main_mark.addProperty(PropertyType._class, "main");
            this.footer_mark.addProperty(PropertyType._class, "footer");
        ...
        public void AddHeader(Mark mark){
            this.header_mark.addChildMark(mark);
        }

        public void AddMain(Mark mark){
            this.main_mark.addChildMark(mark);
        }

        public void AddFooter(Mark mark){
            this.footer_mark.addChildMark(mark);
        }

        public void UseConfig(ConfigConverter configConverter){
            foreach (var headMark in configConverter.head_marks){
                this.addHead(headMark);
            }

            foreach (var headerMark in configConverter.header_marks){
                this.AddHeader(headerMark);
            }

            foreach (var footerMark in configConverter.footer_marks){
                this.AddFooter(footerMark);
            }
        }

        
        public Mark addBody(string pre_content, string post_content, MarkType mt) =>
            this.main_mark.addChildMark(pre_content, post_content, mt);
        ...
        public Mark addBody(Mark mark) => this.main_mark.addChildMark(mark);
